...
use crate::{
    ...
    storage::{
        ...
        image_comp_properties::StageProperties,
                    ...
                    mouse::Event::CursorMoved { position } => {
                        if let Some(drag_start) = state.drag_start
                            && let Some(image_in_canvas) =
                                ImageInCanvas::for_app_state_and_bounds(&self.app_state, bounds)
                        {
                            let image_pos = image_in_canvas.mouse_to_image(position);
                            state.drag_shift = Some(Size::new(
                                image_pos.x - drag_start.x,
                                image_pos.y - drag_start.y,
                            ))
                            .filter(|p| p.width.abs() >= 1e-2 || p.height.abs() >= 1e-2);
                            if let Some(shift) = state.drag_shift {
                                let s_p = &self
                                    .app_state
                                    .storage
                                    .as_ref()
                                    ...
                                    .properties
                                    .stage_properties;
                                let startkoo = s_p.pix_to_math(Point2D::new(
                                    drag_start.x as i32,
                                    drag_start.y as i32,
                                ));
                                let dragkoo = s_p.pix_to_math(Point2D::new(
                                    shift.width as i32,
                                    shift.height as i32,
                                ));
                                println!(
                                    "GGG - startkoo is {:?}, shift is {:?}",
                                    startkoo, dragkoo
                                );
                            }
